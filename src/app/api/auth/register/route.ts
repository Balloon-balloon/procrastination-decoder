  ...
  try {
    ...
    const created = await withDatabase((database) => {
      ...
      const user = {
        ...
        passwordHash: hash, passwordSalt: salt, verified: !needsVerification,
        verificationToken,
        verificationExpiresAt: verificationToken ? new Date(Date.now() + 86400000).toISOString() : undefined,
