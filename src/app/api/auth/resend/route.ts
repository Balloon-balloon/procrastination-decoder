...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await withDatabase((database) => {
      ...
      user.verificationToken = hasEmailService() ? createId() + createId() : undefined;
      user.verificationCode = hasEmailService() ? undefined : generateVerifyCode();
    ...
    return NextResponse.json({
      success: true,
      message: "验证码已重新生成",
      verificationCode: result.user.verificationCode,
    });
  } catch (error) {
